...
with open('tokenJs.json', 'r') as bolo:
     pin = json.load(bolo)
cl = LINE(pin['token'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token'] = cl.authToken
ka = LINE(pin['token2'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token2'] = ka.authToken
kb = LINE(pin['token3'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token3'] = kb.authToken
kc = LINE(pin['token4'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token4'] = kc.authToken
kd = LINE(pin['token5'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token5'] = kd.authToken
ke = LINE(pin['token6'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token6'] = ke.authToken
kf = LINE(pin['token7'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token7'] = kf.authToken
k1 = LINE(pin['token8'],appName='IOS\t9.22.2\tIOS\t13.2.2')
pin['token8'] = k1.authToken

print ("\nじɪᴘʀᴏ ᴠ.3.ᴀ Succes Run ")
...
Ang=["uac4e0127e4e1a4e9ce7af0eeac27efb5","u42626a43e161bd575112e0917f1592e2"]

Bots=[mid,Amid,Bmid,Cmid,Dmid,Emid,Fmid]
Ajs=[Smid]
Angrust = Bots+Ang+Ajs
...
def command(text):
    pesan = text.lower()
    if pesan.startswith(LiproSett["keyCmd"]):
        cmd = pesan.replace(LiproSett["keyCmd"],"")
    else:
        cmd = "command"
    return cmd
...
def bot(op):
      try:
          ...
          global threading
          if op.type == 11:
            if op.param1 in LiproSett["proqr"]:
                if op.param2 in Angrust or op.param2 in LiproSett["Bots"] or op.param2 in LiproSett["admin"]:pass
                else:
          ...
          if op.type == 17:
            if op.param2 in wait["blacklist"]:
                if op.param2 not in Angrust and op.param2 not in LiproSett["Bots"] and op.param2 not in LiproSett["admin"]:
                    try:
          ...
          if op.type == 32:
            if op.param3 in Angrust or op.param3 in LiproSett["admin"] or op.param3 in LiproSett["Bots"]:
                if op.param2 not in Angrust and op.param2 not in LiproSett["Bots"] and op.param2 not in LiproSett["admin"]:
                    wait["blacklist"][op.param2] = True
                    try:
                        ka.kickoutFromGroup(op.param1,[op.param2])
          ...
          if op.type == 19:
            ...
            if op.type == 19:
                ...
                if op.param3 in LiproSett["Bots"]:
                    if op.param2 not in Angrust or op.param2 not in LiproSett["Bots"] or op.param2 not in LiproSett["admin"]:
                        wait["blacklist"][op.param2] = True
                        try:
                            ka.kickoutFromGroup(op.param1,[op.param2])
                            ka.inviteIntoGroup(op.param1,[op.param3])
                ...
                if op.param3 in LiproSett["admin"]:
                    if op.param2 not in Angrust or op.param2 not in LiproSett["Bots"] or op.param2 not in LiproSett["admin"]:
                        wait["blacklist"][op.param2] = True
                        try:
                            ka.kickoutFromGroup(op.param1,[op.param2])
          ...
          if op.type == 5:
            if LiproSett["autoAdd"] == True:
                if op.param2 not in Angrust:
                    cl.findAndAddContactsByMid(op.param1)
                    if (LiproSett["message"] in [" "," ","\n",None]):
                        pass
                    else:
                        cl.sendMessage(op.param1, LiproSett["message"])

          if op.type == 11:
            if op.param1 in LiproSett["intaPoint"]:
                if op.param2 in Angrust and op.param2 in LiproSett["Bots"] and op.param2 not in LiproSett["admin"]:
                    pass
                else:
                    X = cl.getGroup(op.param1)
          ...
          if op.type == 13:
            if mid in op.param3:
                if LiproSett["autoJoin"] == True:
                    if op.param2 not in Angrust and op.param2 not in LiproSett["Bots"] and op.param2 not in LiproSett["admin"]:
                        cl.acceptGroupInvitation(op.param1)
                        cl.leaveGroup(op.param1)
                    else:
                        cl.acceptGroupInvitation(op.param1)
                        if LiproSett["bypas"] == True:
            ...
            if Smid in op.param3:
                if LiproSett["bypas"] == True:
                    if op.param2 not in Angrust and op.param2 not in LiproSett["Bots"] and op.param2 not in LiproSett["admin"]:
                        ...
                        k1.leaveGroup(op.param1)
          if op.type == 13:
            if op.param1 in LiproSett["proInvite"]:
                if op.param2 in Angrust or op.param2 in LiproSett["Bots"] or op.param2 in LiproSett["admin"]:
                    pass
                else:
                    try:
                        if op.param3 in Angrust or op.param3 in LiproSett["Bots"] or op.param3 in LiproSett["admin"]:
                            pass
                        else:
                            if op.param2 not in Angrust and op.param2 not in LiproSett["Bots"] and op.param2 not in LiproSett["admin"]:
                                wait["blacklist"][op.param2] = True
                                ...
                                if anu.invitee is not None:
                                    ...
                                    for target in pipo:
                                      if target in op.param3 and target not in Angrust and target not in LiproSett["Bots"] and target not in LiproSett["admin"]:
                ...
                if op.param3 in Angrust or op.param3 in LiproSett["Bots"] or op.param3 in LiproSett["admin"]:pass
                else:
          ...
          if op.type == 15:
            if op.param1 in LiproSett["leaveMsg"]:
                if op.param2 not in Angrust and op.param2 not in LiproSett["admin"] and op.param2 not in LiproSett["Bots"]:
                    return
                else:
                    cl.sendMessage(op.param1, LiproSett["leftmsg"])

          if op.type == 17:
            if op.param1 in LiproSett["welcome"]:
                if op.param2 in Angrust or op.param2 in LiproSett["Bots"] or op.param2 in LiproSett["admin"]:
                    pass
          ...
          if op.type == 19:
            if op.param1 in LiproSett["proKick"]:
                if op.param2 in Angrust or op.param2 in LiproSett["Bots"] or op.param2 in LiproSett["admin"]:
                    pass
                else:
                    wait["blacklist"][op.param2] = True
          ...
          if op.type == 32:
            if op.param1 in LiproSett["proCancel"]:
                if op.param2 in Angrust or op.param2 in LiproSett["Bots"] or op.param2 in LiproSett["admin"]:
                    pass
                else:
                    wait["blacklist"][op.param2] = True
          ...
          if op.type == 55:
            try:
                if op.param1 in LiproSett["readPoint"]:
                   if op.param2 in LiproSett["readMember"][op.param1]:pass
                   else:LiproSett["readMember"][op.param1][op.param2] = True
                else:pass
            except:pass
          ...
          if op.type in [25,26]:
            ...
            if msg.toType == 0 or msg.toType == 2:
               ...
               if msg.contentType == 13:
                 if msg._from in Ang:
                  if LiproSett["abots"] == True:
                    if msg.contentMetadata["mid"] in LiproSett["Bots"]:
                        cl.sendMessage(msg.to,"ᴡᴀs ʙᴏᴛ ғʀɪᴇɴᴅ")
                    else:
                        LiproSett["Bots"][msg.contentMetadata["mid"]] = True
                        cl.sendMessage(msg.to,"sᴜᴄᴄᴇs ᴀᴅᴅ ʙᴏᴛs")
                 if LiproSett["dbots"] == True:
                    if msg.contentMetadata["mid"] in LiproSett["Bots"]:
                        del LiproSett["Bots"][msg.contentMetadata["mid"]]
                        cl.sendMessage(msg.to,"sᴜᴄᴄᴇs ʀᴇᴍᴏᴠᴇ ʙᴏᴛs")
                    else:
                        cl.sendMessage(msg.to,"ᴄᴏɴᴛᴀᴄᴛ ɴᴏᴛ ɪɴ ʟɪsᴛ ʙᴏᴛs")
                 if msg._from in Ang:
                  if LiproSett["addadmin"] == True:
                    if msg.contentMetadata["mid"] in LiproSett["admin"]:
                        cl.sendMessage(msg.to,"ᴡᴀs ᴀᴅᴍɪɴ")
                    else:
                        LiproSett["admin"][msg.contentMetadata["mid"]] = True
                        cl.sendMessage(msg.to,"sᴜᴄᴄᴇs ᴀᴅᴅ ᴀᴅᴍɪɴ")
                 if LiproSett["deladmin"] == True:
                    if msg.contentMetadata["mid"] in LiproSett["admin"]:
                        del LiproSett["admin"][msg.contentMetadata["mid"]]
                        cl.sendMessage(msg.to,"sᴜᴄᴄᴇs ʀᴇᴍᴏᴠᴇ ᴀᴅᴍɪɴ")
                    else:
                        cl.sendMessage(msg.to,"ᴄᴏɴᴛᴀᴄᴛ ɴᴏᴛ ɪɴ ʟɪsᴛ ᴀᴅᴍɪɴ")
                 if msg._from in Ang:
                  if LiproSett["ablacklist"] == True:
                    if msg.contentMetadata["mid"] in wait["blacklist"]:
                        cl.sendMessage(msg.to,"ᴡᴀs ʙʟᴀᴄᴋʟɪsᴛ")
                    else:
                        wait["blacklist"][msg.contentMetadata["mid"]] = True
                        cl.sendMessage(msg.to,"sᴜᴄᴄᴇs ᴀᴅᴅ ɪɴ ʙʟᴀᴄᴋʟɪsᴛ")
                 if LiproSett["dblacklist"] == True:
                    if msg.contentMetadata["mid"] in wait["blacklist"]:
                    ...
                    for cmd in commandM.split('.'):
                        if msg._from in Ang:
                            if cmd.startswith("1run "):
                               ...
                               if pesan == "":
                                   setKey = "" if not LiproSett['keyCmd'] else LiproSett['keyCmd']
                               else:
                                   setKey = pesan
                               cl.sendMessage(to, str(setKey))
                        if msg._from in Ang or msg._from in LiproSett["admin"]:
                          if cmd == "key":cl.sendMessage(to, Menu.help(keyt))
                          if cmd == "key1":cl.sendMessage(to, Menu.helpset(keyt))
                          if cmd == "key2":cl.sendMessage(to, Menu.helpbot(keyt))
                          ...
                          if cmd == "mykey":
                               cl.sendMessage(msg.to, "ᴋᴇʏ ɴᴏᴡ「 " + str(LiproSett["keyCmd"]) + " 」")
                          if cmd.startswith("setkey "):
                               ...
                               if key in [""," ","\n",None]:
                                   ...
                                   LiproSett["keyCmd"] = str(key).lower()
...
#===========================================#
                                  ...
                                  if int(number) < len(groups) and int(number) >= 0:
                                      ...
                                      for dhe in [ka,kb,kc,kd,ke,kf,cl]:
                                        try:
                                            ...
                                            cl.sendMessage(to,'Sucess Membubarkan Semua Member ..')
                                            if x.preventedJoinByTicket == True:
                                                x.preventedJoinByTicket = False
                                                cl.updateGroup(x)
                          ...
                          if cmd == "lurk on":
                                 tz = pytz.timezone("Asia/Jakarta")
                                 timeNow = datetime.now(tz=tz)
                                 LiproSett['readPoint'][msg.to] = msg_id
                                 LiproSett['readMember'][msg.to] = {}
                                 cl.sendMessage(msg.to, "Lurking berhasil diaktifkan\nTanggal : "+ datetime.strftime(timeNow,'%Y-%m-%d')+"\nJam [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]")
                          if cmd == "lurk off":
                                 tz = pytz.timezone("Asia/Jakarta")
                                 timeNow = datetime.now(tz=tz)
                                 del LiproSett['readPoint'][msg.to]
                                 del LiproSett['readMember'][msg.to]
                                 cl.sendMessage(msg.to, "Lurking berhasil dinoaktifkan\nTanggal : "+ datetime.strftime(timeNow,'%Y-%m-%d')+"\nJam [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]")
                          if cmd == "lurkers":
                            if msg.to in LiproSett['readPoint']:
                                if LiproSett['readMember'][msg.to] != {}:
                                    aa = []
                                    for x in LiproSett['readMember'][msg.to]:
                                        aa.append(x)
                                    try:
                                        arrData = ""
                                        textx = "  [ Result {} member ]    \n  [ Lurkers ]\n1. ".format(str(len(aa)))
                                    ...
                                    try:
                                        ...
                                        pass
                                    LiproSett['readPoint'][msg.to] = msg.id
                                    LiproSett['readMember'][msg.to] = {}
                                else:
                          ...
                          if 'Spam: ' in cmd:
                              ...
                              if jumlah <= 1000:
                                  for var in range(0,jumlah):
                                      ka.sendMessage(midd, str(LiproSett["spamMsg"]))
                                      kb.sendMessage(midd, str(LiproSett["spamMsg"]))
                                      kc.sendMessage(midd, str(LiproSett["spamMsg"]))
                                      kd.sendMessage(midd, str(LiproSett["spamMsg"]))
                                      ke.sendMessage(midd, str(LiproSett["spamMsg"]))
                                      kf.sendMessage(midd, str(LiproSett["spamMsg"]))
                          if 'id line: ' in cmd:
                          ...
                          if 'left ' in cmd:
                              ...
                              if spl == 'on':
                                  if msg.to in LiproSett["leaveMsg"]:
                                       msgs = "ᴡᴀs ᴏɴ"
                                  else:
                                       LiproSett["leaveMsg"][msg.to] = True
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「sᴇɴᴅ ᴍᴇssᴀɢᴇ ʟᴇᴀᴠᴇ」" + msgs)
                              elif spl == 'off':
                                    if msg.to in LiproSett["leaveMsg"]:
                                         del LiproSett["leaveMsg"][msg.to]
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏғғ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                    else:
                                         msgs = "ᴡᴀs ᴏғғ"
                          ...
                          if 'proqr ' in cmd:
                              ...
                              if spl == 'on':
                                  if msg.to in LiproSett["proqr"]:
                                       msgs = "ᴡᴀs ᴏɴ"
                                  else:
                                       LiproSett["proqr"][msg.to] = True
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「ᴘʀᴏᴛᴇᴄᴛɪᴏɴ ʟɪɴᴋ」" + msgs)
                              elif spl == 'off':
                                    if msg.to in LiproSett["proqr"]:
                                         del LiproSett["proqr"][msg.to]
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏғғ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                    else:
                                         msgs = "ᴡᴀs ᴏғғ"
                          ...
                          if 'prokick ' in cmd:
                              ...
                              if spl == 'on':
                                  if msg.to in LiproSett["proKick"]:
                                       msgs = "ᴡᴀs ᴏɴ"
                                  else:
                                       LiproSett["proKick"][msg.to] = True
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「ᴘʀᴏᴛᴇᴄᴛ ᴋɪᴄᴋ」" + msgs)
                              elif spl == 'off':
                                    if msg.to in LiproSett["proKick"]:
                                         del LiproSett["proKick"][msg.to]
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏғғ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                    else:
                                         msgs = "ᴡᴀs ᴏғғ"
                          ...
                          if 'proinvite ' in cmd:
                              ...
                              if spl == 'on':
                                  if msg.to in LiproSett["proInvite"]:
                                       msgs = "ᴡᴀs ᴏɴ"
                                  else:
                                       LiproSett["proInvite"][msg.to] = True
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「ᴘʀᴏᴛᴇᴄᴛ ɪɴᴠɪᴛᴇ」" + msgs)
                              elif spl == 'off':
                                    if msg.to in LiproSett["proInvite"]:
                                         del LiproSett["proInvite"][msg.to]
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏғғ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                    else:
                                         msgs = "ᴡᴀs ᴏғғ"
                          ...
                          if 'procancel ' in cmd:
                              ...
                              if spl == 'on':
                                  if msg.to in LiproSett["proCancel"]:
                                       msgs = "ᴡᴀs ᴏɴ"
                                  else:
                                       LiproSett["proCancel"] = True
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「ᴘʀᴏᴛᴇᴄᴛ ᴄᴀɴᴄᴇʟ」" + msgs)
                              elif spl == 'off':
                                    if msg.to in LiproSett["proCancel"]:
                                         LiproSett["proCancel"][msg.to]
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏғғ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                    else:
                                         msgs = "ᴡᴀs ᴏғғ"
                              ...
                              if spl == 'on':
                                  ...
                                  if msg.to in LiproSett["proCancel"]:
                                      ...
                                      msgs = "ᴡᴀs ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                       ...
                                       LiproSett["proCancel"][msg.to] = True
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「ᴀʟʟ ᴘʀᴏᴛᴇᴄᴛɪᴏɴ」" + msgs)
                                    ...
                                    if msg.to in LiproSett["proInvite"]:
                                         ...
                                         msgs = ""
                                    if msg.to in LiproSett["proCancel"]:
                                         del LiproSett["proCancel"][msg.to]
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏғғ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                    else:
                                         ginfo = cl.getGroup(msg.to)
                          ...
                          if 'inta ' in cmd:
                              ...
                              if spl == 'on':
                                  if msg.to in LiproSett["intaPoint"]:
                                       msgs = "ᴡᴀs ᴏɴ"
                                  else:
                                       LiproSett["intaPoint"][msg.to] = True
                                       ginfo = cl.getGroup(msg.to)
                                       msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏɴ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                  cl.sendMessage(msg.to, "「ɪɴᴛᴀ ᴘᴏɪɴᴛ」" + msgs)
                              elif spl == 'off':
                                    if msg.to in LiproSett["intaPoint"]:
                                         del LiproSett["intaPoint"][msg.to]
                                         ginfo = cl.getGroup(msg.to)
                                         msgs = "ᴀʟʟʀᴇᴀᴅʏ ᴏғғ ɪɴ ɢʀᴏᴜᴘ : " +str(ginfo.name)
                                    else:
                                         msgs = "ᴡᴀs ᴏғғ"
                          ...
                          if "bot kick " in cmd:
                              if 'MENTION' in msg.contentMetadata.keys():
                                  ...
                                  for mention in mentions['MENTIONEES']:
                                      uid = mention['M']
                                      if uid in Ang or uid in Bots or uid in LiproSett["Bots"]:
                                          continue
                                      try:
                          ...
                          if "@cancelin" in cmd:
                               ...
                               if group.invitee is None:
                                   ...
                                   for x in nama:
                                     if x not in Ang and x not in Bots and x not in LiproSett["Bots"] and x not in LiproSett["admin"]:
                          ...
                          if "addadmin " in cmd:
                            if 'MENTION' in msg.contentMetadata.keys()!= None:
                               ...
                               for x in key["MENTIONEES"]:
                                    targets.append(x["M"])
                               for target in targets:
                                   if target in LiproSett["admin"]:
                                       cl.sendMessage(msg.to,"ωαs ıη α∂мıη")
                                   else:
                                       try:
                                           LiproSett["admin"][target] = True
                          ...
                          if "addbot " in cmd:
                            if 'MENTION' in msg.contentMetadata.keys()!= None:
                               ...
                               for x in key["MENTIONEES"]:
                                    targets.append(x["M"])
                               for target in targets:
                                   if target in LiproSett["Bots"]:
                                       cl.sendMessage(msg.to,"ωαs ıη вσтs")
                                   else:
                                       try:
                                           LiproSett["Bots"][target] = True
                          ...
                          if "deladmin " in cmd:
                            if 'MENTION' in msg.contentMetadata.keys()!= None:
                               ...
                               for x in key["MENTIONEES"]:
                                    targets.append(x["M"])
                               for target in targets:
                                   if target not in LiproSett["admin"]:
                                       cl.sendMessage(msg.to,"ησт ıη α∂мıη")
                                   else:
                                       try:
                                           del LiproSett["admin"][target]
                          ...
                          if "delbot " in cmd:
                            if 'MENTION' in msg.contentMetadata.keys()!= None:
                               ...
                               for x in key["MENTIONEES"]:
                                    targets.append(x["M"])
                               for target in targets:
                                   if target not in LiproSett["Bots"]:
                                       cl.sendMessage(msg.to,"ησт ıη ʙᴏᴛs")
                                   else:
                                       try:
                                           del LiproSett["Bots"][target]
                          ...
                          if cmd == "cbot" or text.lower() == 'clear bot':
                              ang = cl.getContacts(LiproSett["Bots"])
                              mc = "%i вστs " % len(ang)
                          ...
                          if cmd == "allrefresh" or text.lower() == 'refresh':
                                ...
                                LiproSett["abots"] = False
                                LiproSett["dbots"] = False
                                LiproSett["ablacklist"] = False
                                LiproSett["dblacklist"] = False
                                LiproSett["LikeOn"] = False
                                cl.sendMessage(msg.to," ╠❂➤  вστs αℓℓrєα∂y ƒrєsн.....❂➤")
                          if cmd == "killban":
                                if msg.toType == 2:
                                    group = cl.getGroup(msg.to)
                          ...
                          if cmd == "cekmsg":
                               cl.sendMessage(msg.to, "「ᴍᴇssᴀɢᴇ ᴀᴅᴅ」 :\n「 " + str(LiproSett["message"]) + " 」")
                               cl.sendMessage(msg.to, "「ᴍᴇssᴀɢᴇ sɪᴅᴇʀ」:\n「 " + str(LiproSett["siderMsg"]) + " 」")
                               cl.sendMessage(msg.to, "「ᴍᴇssᴀɢᴇ ᴍᴇɴᴛɪᴏɴ」:\n「 " + str(LiproSett["msgTag"]) + " 」")
                               cl.sendMessage(msg.to, "「ᴍᴇssᴀɢᴇ sᴘᴀᴍ」:\n「 " + str(LiproSett["spamMsg"]) + " 」")
                               cl.sendMessage(msg.to, "「ᴍᴇssᴀɢᴇ ᴡᴇʟʟᴄᴏᴍᴇ」:\n「 " + str(LiproSett["sambutan"]) + " 」")
                               cl.sendMessage(msg.to, "「ᴍᴇssᴀɢᴇ ʟᴇᴀᴠᴇ」:\n「 " + str(LiproSett["leftmsg"]) + " 」")
#===========JOIN TICKET============#
                          if "/ti/g/" in msg.text.lower():
                              if LiproSett["autoJoinTicket"] == True:
                          ...
                          if cmd == "cleanse":
                               if msg.toType == 2:
                                  ...
                                  for x in nama:
                                      if x not in Angrust:
                                          if x not in LiproSett["Bots"]:
                                              if x not in LiproSett["admin"]:
                                                  try:
                                                      klist=[ka,kb,kc,kd,ke,kf]
...
def siderMembers(to, mid):
    try:
        ...
        for i in mid:
            ...
            arrData = {'S':slen, 'E':elen, 'M':i}
            arr.append(arrData)
            textx += mention+LiproSett["siderMsg"]
            if no < len(mid):
                no += 1
                textx += "%i. " % (num)
                num=(num+1)
...
def welcomeMembers(to, mid):
    try:
        ...
        for i in mid:
            ...
            arrData = {'S':slen, 'E':elen, 'M':i}
            arr.append(arrData)
            textx += mention+LiproSett["sambutan"]+"\ndi group : "+str(ginfo.name)
            if no < len(mid):
                no += 1
                textx += "%i " % (num)
                num=(num+1)
